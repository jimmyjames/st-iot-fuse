...
function Slackbot(configuration) {
    ...
    slack_botkit.createHomepageEndpoint = function(webserver) {
        slack_botkit.log("** Serving app landing page at : http://" + slack_botkit.config.hostname + ":" + slack_botkit.config.port + "/");
    ...
    function secureWebhookEndpoints(webserver, endpoint, tokens) {
        ...
        slack_botkit.log(
            "** Requiring token authentication for webhook endpoints for Slash commands " +
                "and outgoing webhooks; configured " +
                tokens.length +
                " token(s)"
        );
        ...
        slack_botkit.findTeamById(team_id, function(err, team) {
            if (team) {
                ...
                if (payload.authed_teams) {
                    async.eachSeries(
                        payload.authed_teams,
                        function(team_id, next) {
                            slack_botkit.findTeamById(team_id, function(err, team) {
                                if (team) {
                                    found_team = team;
                                    next();
    ...
    slack_botkit.middleware.format.use(function formatForSlack(bot, message, platform_message, next) {
        ...
        platform_message.attachments = message.attachments ? JSON.stringify(message.attachments) : null;
    ...
    slack_botkit.getAuthorizeURL = function(team_id, redirect_params) {
        ...
        var url =
            api_root + "/oauth/authorize" + "?client_id=" + slack_botkit.config.clientId + "&scope=" + scopes.join(",") + "&state=botkit";

        if (team_id) url += "&team=" + team_id;
    ...
    slack_botkit.createOauthEndpoints = function(webserver, callback) {
        slack_botkit.log("** Serving login URL: http://" + slack_botkit.config.hostname + ":" + slack_botkit.config.port + "/login");
        ...
        var call_api = function(command, options, cb) {
            ...
            request
                .post(api_root + "/api/" + command, function(error, response, body) {
        ...
        slack_botkit.log(
            "** Serving oauth return endpoint: http://" + slack_botkit.config.hostname + ":" + slack_botkit.config.port + "/oauth"
        );
